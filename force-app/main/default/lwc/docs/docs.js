...
export default class CandidateDocumentUpload extends LightningElement {
    ...
    ---------------------------- */
    async handleSubmit() {
        // ensure at least one file selected among the displayed inputs
        ...
        try {
            ...
            // Upload each selected file via Apex uploadFile and collect ContentDocumentIds
            for (const item of this.missingUploadItems) {
                ...
                if (file) {
                    ...
                    // store returned ContentDocumentId for saveDocuments
                    this.contentDocIds[key] = contentDocumentId;
                }
            }

            // Now call your existing saveDocuments Apex method with the proper params
            await saveDocuments({
                name: this.name,
                email: this.email,
                phone: this.phone,
            ...
            // reset form (preserve original flow)
            this.resetForm();
        } catch (error) {
